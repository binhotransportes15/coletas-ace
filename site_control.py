...
O site consulta esse arquivo e mostra tela de interrupcao (overlay).
"""
...
def publish_site_status(
    *,
    ...
    add = _run(["git", "add", "dashboard/status.json", "dashboard/index.html", "dashboard/offline.html"])
    if add.returncode != 0:
    ...
    return (
        ...
        "Em ate ~5s a TV/site mostra a tela INTERROMPIDO (Ctrl+F5 se cache)."
    )
